...
mutable struct PolyNomCirc{W,T}
...
struct geobucketpol1{W}
    bucket::Vector{PolyNomCirc{W,FieldElem}}
end
...
"""
function DIVCirc(f::PolyNomCirc{W,FieldElem},G::Vector{PolyNomCirc{W,FieldElem}}) where W
...
"""
function pushing(r::PolyNomCirc{W,FieldElem},LTf2M,LTf2K) where{W}
    if capacity(r.coefficients) > length(r.coefficients)
        ...
        Nemo.set!(r21.buffer[r.monoms.n],LTf2K)
        r21.last +=1
        r21.n +=1
    end
...
"""
function add(f::PolyNomCirc{W,FieldElem},g::PolyNomCirc{W,FieldElem},DIV1,DIV2)where{W}
...
"""
function add(f::PolyNomCirc{W,FieldElem},g::PolyNomCirc{W,FieldElem})where{W}
    lf = length(f.coefficients)
    lg = length(g.coefficients)
    k= 1
    j=1
